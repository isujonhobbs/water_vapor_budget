...
fgrp = Dataset(gfl)
gskw = fgrp.variables['CatKur_Quantile'][:,:] - 3.0
...
fgrp = Dataset(gfl)
cskw = fgrp.variables['CatKur_Quantile'][:,:] - 3.0
fgrp.close()

# Open up Data File
cfl = "../Data/ObsMCMCDat_VAR_201007.nc"
fgrp = Dataset(cfl,"r")
obskew = fgrp.variables['CatKur'][:] - 3.0
fgrp.close()
...
p1.set_ylim(-2,15)
p1.set_xlim(-0.5,8.5)
p1.set_ylabel('Excess Kurtosis')
p1.set_xlabel('Divergence Category [mm]')
p1.yaxis.set_major_locator(mticker.FixedLocator(numpy.arange(-2,16,2)))
p1.xaxis.set_major_locator(mticker.FixedLocator(cts))
p1.set_xticklabels(dlb2)

for i in range(len(xst)):
    rect = pyplot.Rectangle((xst[i],yst[i]),1,17, \
...
p1.plot([4.0,4.0],[-2,17],'k-',lw=1.2, zorder=5)
...
leg = p1.legend((oln,gln,cln),('Obs','Spatial Variance','Constant'),'upper center', \
                numpoints=1, \
...
pyplot.title('Empirical Conditional Kurtosis',size=12)
fig.savefig('PPComp_ExKur_201007.pdf')
...
#p1.set_ylabel('Excess Kurtosis')
#p1.set_xlabel('Divergence Category [mm]')
#p1.yaxis.set_major_locator(mticker.FixedLocator(numpy.arange(-2,12,2)))
#p1.xaxis.set_major_locator(mticker.FixedLocator(cts))
#p1.set_xticklabels(dlb2)

#for i in range(len(xst)):
#    rect = pyplot.Rectangle((xst[i],yst[i]),1,12, \
...
#p1.yaxis.grid(color='#777777',linestyle='dotted')
#p1.plot([-0.5,11.5],[0,0],'k-',lw=1.2, zorder=5)
#p1.plot([5.5,5.5],[-2,10],'k-',lw=1.2, zorder=5)
...
#            zorder=8,markersize=8,capsize=8,elinewidth=1.25,markeredgewidth=1.25)
...
#pyplot.title('Excess Kurtosis',size=12)
#fig.savefig('PPComp_ExKur_Scl_201007.pdf')
