...
fgrp = Dataset(gfl)
gskw = fgrp.variables['CatMean_Quantile'][:,:]
...
fgrp = Dataset(gfl)
cskw = fgrp.variables['CatMean_Quantile'][:,:]
fgrp.close()

# Open up Data File
cfl = "../Data/ObsMCMCDat_VAR_201007.nc"
fgrp = Dataset(cfl,"r")
obskew = fgrp.variables['CatMean'][:]
fgrp.close()
...
p1.set_ylim(-3,3)
p1.set_xlim(-0.5,8.5)
p1.set_ylabel('Mean [mm]')
p1.set_xlabel('Divergence Category [mm]')
p1.yaxis.set_major_locator(mticker.FixedLocator(numpy.arange(-3,3,1)))
...
pyplot.title('Empirical Conditional Mean',size=12)
fig.savefig('PPComp_Mean_201007.pdf')
...
#p1.set_ylabel('Mean')
#p1.set_xlabel('Divergence Category [mm]')
#p1.yaxis.set_major_locator(mticker.FixedLocator(numpy.arange(-10,12,2)))
#p1.xaxis.set_major_locator(mticker.FixedLocator(cts))
#p1.set_xticklabels(dlb2)
...
#p1.yaxis.grid(color='#777777',linestyle='dotted')
#p1.plot([-0.5,11.5],[0,0],'k-',lw=1.2, zorder=5)
#p1.plot([5.5,5.5],[-10,10],'k-',lw=1.2, zorder=5)
...
#pyplot.title('Mean',size=12)
#fig.savefig('PPComp_Mean_Scl_201007.pdf')
